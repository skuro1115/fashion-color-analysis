...
| `…_hue` `…_sat` `…_bri` | **色相**（0〜360°、赤=0・黄=60・緑=120・青=240）・**彩度**（鮮やかさ 0〜100）・**明度**（明るさ 0〜100）。Photoshop や Figma の HSB と同じ。黒・白・グレーは色相が空欄（表の右端にあります） |
...
- 色相を平均するときは注意：359° と 1° はどちらも赤ですが、単純に平均すると 180°（水色）になります。
  年代ごとの傾向を見るなら「彩度・明度の平均」や「色相の分布（何°台が何枚）」で比べるのがおすすめです。
